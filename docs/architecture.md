...
`policy-config`는 다양한 설정 소스에서 정책 값을 읽어오는 순수 1계층 OSS다.
...
- 멀티모듈은 배포와 책임 경계를 위한 구조이며 아키텍처 계층이 아니다.
...
세 모듈은 모두 같은 라이브러리 계층에 속한다. 서비스 계층, 도메인 정책 실행 계층, 프레임워크 어댑터 계층은 포함하지 않는다.
...
- Spring은 포함하지 않는다.
- 외부 저장소, DB, HTTP 클라이언트, 서비스별 fallback 같은 운영 어댑터를 포함하지 않는다.
